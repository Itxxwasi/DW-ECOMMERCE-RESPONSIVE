...
router.post('/', adminAuth, async (req, res) => {
    try {
        console.log('🟢 POST /api/products payload:', {
            ...
            description: req.body.description ? req.body.description.substring(0,200) : req.body.description,
            image: req.body.image || req.body.imageFileId || null
        });
        // Validate required fields
        if (!req.body.name || !req.body.name.trim()) {
            console.log('🔴 Validation failed: Product name is required', { payloadSnippet: { name: req.body.name } });
            return res.status(400).json({ message: 'Product name is required' });
        }

        if (!req.body.description || !req.body.description.trim()) {
            console.log('🔴 Validation failed: Product description is required', { payloadSnippet: { description: req.body.description } });
            return res.status(400).json({ message: 'Product description is required' });
        }

        if (req.body.price === undefined || req.body.price === null || req.body.price === '') {
            console.log('🔴 Validation failed: Product price is required', { payloadSnippet: { price: req.body.price } });
            return res.status(400).json({ message: 'Product price is required' });
        }

        const price = parseFloat(req.body.price);
        if (Number.isNaN(price) || price < 0) {
            console.log('🔴 Validation failed: Product price invalid', { payloadSnippet: { price: req.body.price } });
        ...
        if (req.body.stock === undefined || req.body.stock === null || req.body.stock === '') {
            console.log('🔴 Validation failed: Stock quantity is required', { payloadSnippet: { stock: req.body.stock } });
            return res.status(400).json({ message: 'Stock quantity is required' });
        }

        const stock = parseInt(req.body.stock, 10);
        if (Number.isNaN(stock) || stock < 0) {
            console.log('🔴 Validation failed: Stock quantity invalid', { payloadSnippet: { stock: req.body.stock } });
        ...
        if (Number.isNaN(discount) || discount < 0 || discount > 100) {
            console.log('🔴 Validation failed: Discount invalid', { payloadSnippet: { discount: req.body.discount } });
            return res.status(400).json({ message: 'Discount must be a number between 0 and 100' });
        }

        // Flexible validation: either brand OR category is required
        const hasBrand = req.body.brand !== undefined && req.body.brand !== null && req.body.brand !== '';
        const hasCategory = req.body.category !== undefined && req.body.category !== null && req.body.category !== '';

        if (!hasBrand && !hasCategory) {
            console.log('🔴 Validation failed: Either brand OR category is required', { payloadSnippet: { brand: req.body.brand, category: req.body.category } });
            return res.status(400).json({ message: 'Either brand OR category is required' });
        }

        // If category provided, validate it and its department
        let validatedCategory = null;
        let validatedDepartment = null;
        if (hasCategory) {
            validatedCategory = await Category.findById(req.body.category);
            if (!validatedCategory) {
                console.log('🔴 Validation failed: Invalid category', { payloadSnippet: { category: req.body.category } });
                return res.status(400).json({ message: 'Invalid category' });
            }
            if (!validatedCategory.department) {
                console.log('🔴 Validation failed: Category missing department', { payloadSnippet: { category: req.body.category, validatedCategory } });
                return res.status(400).json({ message: 'Category does not have an associated department' });
            }
            validatedDepartment = validatedCategory.department;
        }
        ...
        // Add category/department only if category was provided and validated
        if (validatedCategory) {
            productData.category = validatedCategory._id;
            productData.department = validatedDepartment;
        }
        ...
        if (!product.sections || product.sections.length === 0) {
            ...
            product.sections = [...new Set(defaultSections)];
