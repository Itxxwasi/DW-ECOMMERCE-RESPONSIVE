...
router.get('/', adminAuth, async (req, res) => {
    try {
        ...
        // Support filtering to only brands created from BrandSection
        const query = {};
        if (req.query.fromSection === 'true') {
            query.brandSection = { $exists: true, $ne: null };
            query.isActive = true;
        }
...
router.get('/public', async (req, res) => {
    try {
        ...
        const brands = await Brand.find({ isActive: true })
            .populate('imageUpload', 'url')
            .sort({ order: 1, createdAt: -1 })
            .select('name image alt link order discount discountText _id');
        ...
        brands.forEach(brand => {
            ...
            // Log brand details including discount info
            console.log(`   Brand "${brand.name}": image="${brand.image || 'NO IMAGE'}", discount=${brand.discount || 0}, discountText="${brand.discountText || ''}"`);
        });
...
router.post('/', adminAuth, async (req, res) => {
    try {
        // Validation
        const { name, image, imageFileId, alt, link, order, discount, discountText, isActive, brandSection } = req.body;
        ...
        const brand = new Brand({
            ...
            brandSection: brandSection || undefined
        });
...
router.put('/:id', adminAuth, async (req, res) => {
    try {
        ...
        if (brandSection !== undefined) {
            brand.brandSection = brandSection || undefined;
        }
