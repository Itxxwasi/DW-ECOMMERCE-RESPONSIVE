...
(function() {
    ...
    const initialize = async () => {
        try {
            ...
            // Homepage sections are loaded by home.js automatically when DOM is ready
            
            // Helper to safely load async functions
            const safeLoad = (fn, name) => {
                if (typeof fn === 'function') {
                    try {
                        const result = fn();
                        if (result && typeof result.catch === 'function') {
                            return result.catch(err => console.warn(`${name} loading failed:`, err));
                        }
                        return Promise.resolve(result);
                    } catch (err) {
                        console.warn(`${name} execution failed:`, err);
                        return Promise.resolve();
                    }
                }
                // console.warn(`${name} function not found`); // Optional: silence this warning if functions are optional
                return Promise.resolve();
            };
            ...
            scheduleInit(() => {
                Promise.all([
                    safeLoad(loadDepartments, 'Departments'),
                    safeLoad(loadStaticBrands, 'Brands'),
                    safeLoad(loadCartCount, 'Cart count'),
                    safeLoad(loadFallbackHomepageProducts, 'Homepage products'),
                    ...
                    // Load navbar categories (from header-navbar.js)
                    if (typeof window.loadCategoriesForNavbar === 'function') {
                        window.loadCategoriesForNavbar();
                    }
...
function initialiseMobileMenu() {
    try {
        ...
        // Handle dropdown expansion in both desktop and mobile menus
        // Use event delegation that works for both cloned items and existing items
        $(document).off('click.menuDropdown').on('click.menuDropdown', 
            '.menu-item.has-children > a, ' +
            '.nav-item.has-children > a, ' +
            '.has-children > a, ' +
            '.nav-item-category > a, ' +
            '.mobile-menu-list .menu-item.has-children > a, ' +
            '.mobile-menu-list .nav-item.has-children > a, ' +
            '.mobile-menu-list .has-children > a, ' +
            '.mobile-menu-list .nav-item-category > a', 
        function(e) {
            const $link = $(this);
            ...
            // For mobile, close other open dropdowns when opening a new one
            if (!wasExpanded && $menuItem.closest('.mobile-menu-list').length) {
                $menuItem.siblings('.expanded').removeClass('expanded');
            }
            ...
            console.log('Menu dropdown clicked:', {
                ...
                dropdownItems: $dropdownMenu.find('> li').length,
                isMobile: $menuItem.closest('.mobile-menu-list').length > 0,
...
// DISABLED - Homepage sections removed
async function loadFallbackHomepageProducts() {
    // Homepage sections removed - function disabled
    return Promise.resolve();
    try {
...
async function loadStaticBrands() {
    try {
        ...
        brandsArray.forEach(brand => {
            ...
            link.href = id ? `/brand-products.html?brand=${encodeURIComponent(id)}&name=${encodeURIComponent(name)}` : (brandLink || '#');
        ...
        if (error && (error.name === 'AbortError' || /aborted|ERR_ABORTED|The user aborted a request/i.test(msg))) {
            console.warn('Brand load aborted');
            return;
        }
...
async function loadDepartments() {
    try {
        ...
        // Note: Categories for navbar are loaded by header-navbar.js, not here
...
async function loadCategoriesForNavbar() {
    ...
    try {
        ...
        activeCategories.forEach(category => {
            ...
            // Create menu item (support .nav-item and .menu-item structures)
            ...
            if (hasSubcategories) {
                ...
                // Create dropdown menu (support .menu-item and .nav-item structures)
                let dropdownMenu;
                if (useMenuItemClass) {
...
function renderSearchResults(data, dropdown) {
    ...
    if (products && products.length > 0) {
        ...
        products.forEach(product => {
            ...
            if (productDiscount > 0 && productPrice > 0) {
                ...
                priceHtml = `<div class="search-result-item-price">Rs.${productPrice.toFixed(2)}</div>`;
            } else {
                priceHtml = '<div class="search-result-item-price">Price not available</div>';
            }
            
            html += `
                <div class="search-result-item" data-product-id="${productId}" onclick="window.location.href='${productUrl}'">
                    <img src="${imageUrl}" alt="${escapeHtml(product.name)}" onerror="this.src='/images/placeholder.png'">
                    <div class="search-result-item-content">
                        <div class="search-result-item-name">${escapeHtml(product.name)}</div>
                        ${priceHtml}
                    </div>
                </div>
            `;
    ...
    if (products.length === 0 && categories.length === 0 && departments.length === 0 && subcategories.length > 0) {
        subcategories.forEach(subcategory => {
            const imageUrl = subcategory.image || subcategory.imageUpload?.url || '/images/placeholder.png';
            const subcategoryId = subcategory._id || subcategory.id;
            const subcategoryUrl = subcategory.url || `/subcategory.html?id=${subcategoryId}`;
            html += `
                <div class="search-result-item" onclick="window.location.href='${subcategoryUrl}'">
                    <img src="${imageUrl}" alt="${escapeHtml(subcategory.name)}" onerror="this.src='/images/placeholder.png'">
                    <div class="search-result-item-content">
                        <div class="search-result-item-name">${escapeHtml(subcategory.name)}</div>
                        <div class="search-result-item-price" style="color: #666; font-weight: 400; font-size: 12px;">Subcategory</div>
                    </div>
                </div>
            `;
        });
...
function initialiseGlobalDelegates() {
    ...
    // Use passive: false only for handlers that need preventDefault
        ...
        if (addButton) {
            ...
            if (productId) {
                ...
                if (productId && productId !== 'undefined' && productId !== 'null') {
                    // Call handler directly without await to prevent blocking
                    handleAddToCart(productId).catch(error => {
...
async function fetchJSON(url, options) {
    ...
    if (typeof fetchOpts.keepalive === 'undefined') {
    ...
    try {
        ...
        if (err && (err.name === 'AbortError' || /aborted|ERR_ABORTED/i.test(msg))) {
            throw err;
        }
        throw err;
    }
...
async function loadFooter() {
    try {
        ...
        const msg = String(error && error.message || '');
        if (error && (error.name === 'AbortError' || /aborted|ERR_ABORTED|The user aborted a request/i.test(msg))) {
            console.warn('Footer load aborted');
            return;
        }
