 $(document).ready(function() {
    ...
    $('#add-product-btn').click(function() {
        ...
        loadAdminBrandsToSelect('#productBrandSection');
...
$('#add-video-banner-to-homepage-btn').click(async function() {
    try {
        ...
        loadHomepageSections();
        // If we just saved a BrandSection, refresh product-brand dropdowns
        try {
            if (sectionType === 'brandSection' && payload && payload.config && Array.isArray(payload.config.brands)) {
                // Capture the last added brand name (most likely the one admin just added)
                const lastBrand = payload.config.brands[payload.config.brands.length - 1];
                const lastBrandName = lastBrand && lastBrand.name ? String(lastBrand.name).trim() : null;

                // Reload brands and then attempt to auto-select the newly created brand by name
                const brands = await loadAdminBrandsToSelect('#productBrandSection');
                // Also refresh other selects that might use brands
                await loadAdminBrandsToSelect('#brand');

                if (lastBrandName && Array.isArray(brands)) {
                    const match = brands.find(b => String(b.name).trim().toLowerCase() === lastBrandName.toLowerCase());
                    if (match && match._id) {
                        try {
                            $('#productBrandSection').val(match._id);
                            console.log(`Auto-selected new brand in product dropdown: ${match.name} (${match._id})`);
                        } catch (e) {
                            console.warn('Unable to auto-select brand in product dropdown:', e);
                        }
                    }
                }
            } else {
                // Generic refresh for other section types
                loadAdminBrandsToSelect('#productBrandSection');
                loadAdminBrandsToSelect('#brand');
            }
        } catch (e) {
            console.warn('Failed to refresh product brand dropdowns:', e);
        }
...
// Load admin-managed Brand documents into a select (uses Brand model)
function loadAdminBrandsToSelect(selectId, options = {}) {
    const { selectedId } = options || {};
    const token = localStorage.getItem('token');

    console.log('Loading admin brands for selector:', selectId);

    return $.ajax({
        // Request only brands that were created from BrandSection entries
        url: '/api/admin/brands?fromSection=true',
        method: 'GET',
        dataType: 'json',
        headers: {
            'x-auth-token': token || ''
        ...
        console.error('Error loading admin brands - AJAX failed:', error);
        $(selectId).html('<option value="">Unable to load brands</option>');
        return [];
    });
}
...
function resetProductForm() {
    ...
    $('#productSubcategory').html('<option value="">Select Category First</option>');
    // Load admin brands into the brand select used elsewhere
    loadAdminBrandsToSelect('#brand');
...
async function saveProduct() {
    ...
    try {
        ...
        const brandSelect = document.getElementById('productBrandSection');
...
async function saveBrandSectionInline(e, capturedName = null) {
    ...
    if (isEdit) {
        ...
        const existingBrand = window.brandSectionBrands[editIndex] || {};
        if (!imageFileId && existingBrand) {
            brandData.imageFileId = existingBrand.imageFileId;
        }
        // Preserve _id if present
        if (existingBrand && existingBrand._id) {
            brandData._id = existingBrand._id;
        }
        ...
        // Sync update to Brand document if it exists, otherwise create it
        (async function syncUpdatedBrand(bData) {
            try {
                const sectionId = $('#homepageSectionId').val();
                const payload = {
                    name: bData.name,
                    image: bData.imageUrl,
                    imageFileId: bData.imageFileId || undefined,
                    link: bData.link || undefined,
                    order: bData.order || 0,
                    discount: bData.discount || 0,
                    discountText: bData.discountText || '',
                    isActive: true,
                    brandSection: sectionId || undefined
                };

                if (bData._id) {
                    // Update existing Brand
                    await $.ajax({
                        url: `/api/admin/brands/${bData._id}`,
                        method: 'PUT',
                        contentType: 'application/json',
                        data: JSON.stringify(payload)
                    });
                    console.log('Synced updated Brand (PUT):', bData._id);
                } else {
                    // Create new Brand
                    ...
                    bData._id = created._id;
                    // append option to selects
                    try {
                        const opt = `<option value="${created._id}">${created.name}</option>`;
                        $('#productBrandSection').append(opt);
                        $('#brand').append(opt);
                    } catch (e) { console.warn('Unable to append created brand option:', e); }
                    console.log('Created Brand for edited inline brand:', created._id);
                }
            } catch (err) {
                console.error('Error syncing updated inline Brand:', err);
            }
        })(brandData);
    } else {
        ...
        // Also create the Brand document immediately so it appears in product-brand dropdowns
        (async function createAdminBrand(bData) {
            try {
                const sectionId = $('#homepageSectionId').val();
                const payload = {
                    name: bData.name,
                ...
                // store returned _id on local brand data for later reference
                bData._id = created._id || created._id;

                // Add newly created brand option to product brand selects and select it
                try {
                    const opt = `<option value="${created._id}">${created.name}</option>`;
                    $('#productBrandSection').append(opt);
                    $('#brand').append(opt);
                    $('#productBrandSection').val(created._id);
                    console.log('Created admin Brand and auto-selected in product dropdown:', created);
                } catch (e) {
                    console.warn('Unable to append new brand option to selects:', e);
                }
            } catch (err) {
                console.error('Error creating admin Brand from inline BrandSection add:', err);
            }
        })(brandData);
    }
...
function updateBrandSectionBrandsList() {
    ...
    $('.remove-brand-section-item').off('click').on('click', function() {
        ...
        if (confirm('Are you sure you want to remove this brand?')) {
            const removed = window.brandSectionBrands[index];
            window.brandSectionBrands.splice(index, 1);
            updateBrandSectionBrandsList();
            showAlert('Brand removed successfully', 'success');

            // If this brand has a saved Brand _id, mark it inactive on the server
            (async function deactivateBrand(r) {
                try {
                    if (r && r._id) {
                        await $.ajax({
                            url: `/api/admin/brands/${r._id}`,
                            method: 'PUT',
                            contentType: 'application/json',
                            data: JSON.stringify({ isActive: false })
                        });
                        console.log('Deactivated admin Brand:', r._id);
                    }
                } catch (err) {
                    console.error('Error deactivating Brand after removal from section:', err);
                }
            })(removed);
        }
    });
        ...
        case 'brandSection':
            ...
            if (config && config.brands && Array.isArray(config.brands) && config.brands.length > 0) {
                ...
                window.brandSectionBrands = config.brands.map(b => ({
                    _id: b._id || null,  // Preserve _id if present so edits/removals can sync
                    name: b.name || '',
                    imageUrl: b.imageUrl || '',
