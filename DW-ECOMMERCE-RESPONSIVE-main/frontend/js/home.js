...
async function fetchWithRetry(url, options = {}) {
    ...
    for (let i = 0; i < attempts; i++) {
        try {
            const opts = { ...options };
            if (typeof opts.keepalive === 'undefined') opts.keepalive = true;
...
async function loadTemplate(templateName) {
    ...
    try {
        ...
        if (error && (error.name === 'AbortError' || /aborted|ERR_ABORTED|The user aborted a request/i.test(msg))) {
            console.warn(`Template load aborted: ${templateName}`);
            return null;
        }
            ...
            switch (section.type) {
                ...
                case 'heroSlider':
                    ...
                    return {
                        ...
                        autoplayInterval: section.config.autoplayInterval || section.config.autoplaySpeed || 3000
                    };
                ...
                case 'brandsection':
                    ...
                    let brandIdLookup = {};
                    try {
                        const resp = await fetch('/api/brands/public');
                        if (resp.ok) {
                            const list = await resp.json();
                            if (Array.isArray(list)) {
                                list.forEach(br => {
                                    const key = String(br.name || br.alt || '').trim().toLowerCase();
                                    if (key) brandIdLookup[key] = br._id || br.id;
                                });
                            }
                        }
                    } catch (e) {}
                    ...
                    const brandsData = sortedBrands.map(b => {
                        ...
                        const inferredId = brandIdLookup[nameKey];
...
async function loadAndRenderHomepageSections() {
    ...
    try {
        ...
        if (typeof document !== 'undefined' && document.visibilityState === 'hidden') {
            console.log('⏭️ Page is navigating away; skipping sections load');
            return;
        }
            ...
            for (const section of sortedSections) {
                ...
                try {
                    ...
                    if (html) {
                        ...
                        if (sectionElement) {
                            // Handle announcement bar (scrollingText) - place at top before header
                            // Only place scrollingText sections with location "top" in the announcement container
                            if (section.type === 'scrollingText' && section.config && section.config.location === 'top') {
                                const announcementContainer = document.getElementById('announcement-bar-container');
                                if (announcementContainer) {
                                    announcementContainer.appendChild(sectionElement);
                                    console.log(`✅ Placed announcement bar in announcement-bar-container (location: top)`);
                                    renderedCount++;
                                    continue;
                                }
                            }
                            ...
                            if (section.type === 'heroSlider' && section.config) {
                                ...
                                sectionElement.setAttribute('data-autoplay-interval', section.config.autoplayInterval || section.config.autoplaySpeed || 3000);
                            }
                            ...
                            // Only append to container if not already placed (e.g., announcement bar)
                            if (sectionElement.parentNode === null || sectionElement.parentNode === tempDiv) {
